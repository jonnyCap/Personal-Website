// canvas - Pure Atmospheric Particle Cloud Swarm & Ultra-Smooth Emergence with Adjusted Minimum Cloud Size
...
(function () {
    ...
    class CloudSwarm {
        constructor(w, h, startX, startY, customScale, shouldEmerge = false, archetypeIndex) {
            ...
            this.targetScale = customScale !== undefined ? customScale : c_random(0.85, 1.40);
    ...
    const simulation = {
        ...
        resize: function () {
            ...
            const w = parent && parent.offsetWidth > 100 ? parent.offsetWidth : 900;
            const h = parent && parent.offsetHeight > 100 ? parent.offsetHeight : 600;
        ...
        init: function () {
            ...
            const w = canvas.width > 0 ? canvas.width : 900;
            const h = canvas.height > 0 ? canvas.height : 600;

            const initialConfigs = [
                { x: w * 0.18, y: h * 0.24, s: 0.90, arch: 4 },
                { x: w * 0.55, y: h * 0.18, s: 1.30, arch: 0 },
                { x: w * 0.35, y: h * 0.65, s: 1.05, arch: 1 }
            ];
        ...
        spawnIncomingCloud: function (w, h, immediateOnScreen = false) {
            const bands = [h * 0.18, h * 0.35, h * 0.55, h * 0.72];
            ...
            for (let band of bands) {
                ...
                for (let s of this.swarms) {
                    if (s.state !== 'dissolving' && Math.abs(s.y - band) < 100) {
                        conflict++;
                    }
                }
